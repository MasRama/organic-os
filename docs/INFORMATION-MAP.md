...
| Editorial-oversight threshold default (7) and factor list (claims density, YMYL adjacency, competitor mentions, legal/compliance surface, verbatim research survival) | `editorial:` block comment in `plugin/lib/core/templates/site-profile.yaml` (default); oversight step in `plugin/agents/ce-editor.md` (factors) | `plugin/skills/onsite-publish/SKILL.md` gate step; `plugin/docs/site-repo-contract.md` editorial key bullet | manual |
